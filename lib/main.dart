          ...
          child: Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                _buildNavItem(icon: Icons.assignment, label: "Assessments", index: 1),
...
class HomeContent extends StatelessWidget {
              ...
              child: Column(
                ...
                children: [
                  ...
                  Container(
                    ...
                    width: 180,
                    fit: BoxFit.contain,
                    errorBuilder: (context, error, stackTrace) => const Icon(Icons.public, size: 60, color: Color(0xFF005DA8)),
                  ),
                    ),
                  ),
                  const SizedBox(height: 16),
                  const Text("WHO Assessment Tool", textAlign: TextAlign.center, style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFF003D73))),
                  const SizedBox(height: 8),
                  Text("Version 1.0.0-beta", style: TextStyle(color: Colors.grey.shade600, fontSize: 13)),
      ...
      children: [
        Column(
          children: [
            ...
            Container(
              width: double.infinity,
              padding: const EdgeInsets.only(top: 80, bottom: 64, left: 24, right: 24),
              ...
              child: Center(
                child: Container(
                  ...
                  child: Padding(
                    padding: const EdgeInsets.all(32.0), // Spazio bianco interno
                    child: Image.asset(
                      'assets/images/who_logo.png',
                      fit: BoxFit.contain,
                      errorBuilder: (context, error, stackTrace) => const Icon(Icons.public, size: 80, color: Color(0xFF005DA8)),
                    ),
                  ),
                ),
              ),
              ...
              child: Transform.translate(
                ...
                child: Center(
                  child: ConstrainedBox(
                    constraints: const BoxConstraints(maxWidth: 600), // LARGHEZZA MAX PER IL WEB
                    child: ListView(
                      padding: const EdgeInsets.symmetric(horizontal: 16),
                      children: [
                        _buildHTMLCard(
                          context: context, title: "Mpox Outbreak", subtitle: "Clade I / Clade II guidelines", type: EmergencyType.mpox,
                          icon: Icons.coronavirus_outlined, iconColor: const Color(0xFF9333EA), iconBgColor: const Color(0xFFFAF5FF),
                          borderColor: const Color(0xFFA855F7), badgeText: "Active", badgeColor: const Color(0xFF7E22CE), badgeBgColor: const Color(0xFFF3E8FF), isActive: true,
                        ),
                        const SizedBox(height: 16),
                        _buildHTMLCard(
                          context: context, title: "Ebola Virus Disease", subtitle: "Filovirus structural guidelines", type: EmergencyType.ebola,
                          icon: Icons.biotech, iconColor: const Color(0xFFDC2626), iconBgColor: const Color(0xFFFEF2F2),
                          borderColor: const Color(0xFFF87171), badgeText: "Soon", badgeColor: const Color(0xFF475569), badgeBgColor: const Color(0xFFE2E8F0), isActive: false,
                        ),
                        const SizedBox(height: 16),
                        _buildHTMLCard(
                          context: context, title: "SARI / COVID-19", subtitle: "Respiratory isolation centers", type: EmergencyType.sars,
                          icon: Icons.masks, iconColor: const Color(0xFF0D9488), iconBgColor: const Color(0xFFF0FDFA),
                          borderColor: const Color(0xFF2DD4BF), badgeText: "Soon", badgeColor: const Color(0xFF475569), badgeBgColor: const Color(0xFFE2E8F0), isActive: false,
                        ),
                      ],
                    ),
                  ),
  ...
  Widget _buildHTMLCard({
    required BuildContext context, required String title, required String subtitle, required EmergencyType type,
    required IconData icon, required Color iconColor, required Color iconBgColor, required Color borderColor,
    required String badgeText, required Color badgeColor, required Color badgeBgColor, required bool isActive,
  }) {
    return Opacity(
      ...
      child: Container(
        ...
        child: Material(
          ...
          child: InkWell(
            ...
            onTap: isActive
                ? () => Navigator.push(context, MaterialPageRoute(builder: (context) => FacilitySelectionScreen(emergency: type)))
                : () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Module currently locked."))),
            child: Padding(
              ...
              child: Row(
                children: [
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          ...
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Flexible(child: Text(title, style: const TextStyle(color: Color(0xFF0F172A), fontWeight: FontWeight.bold, fontSize: 17))),
                            Container(
                              padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                              decoration: BoxDecoration(color: badgeBgColor, borderRadius: BorderRadius.circular(20)),
                              child: Text(badgeText.toUpperCase(), style: TextStyle(color: badgeColor, fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 0.5)),
                            ),
                          ],
                        ),
                        const SizedBox(height: 4),
                  ...
                  Icon(isActive ? Icons.chevron_right : Icons.lock_outline, color: const Color(0xFFCBD5E1), size: 28),
                ],
              ),
            ),
          ),
...
class FacilitySelectionScreen extends StatelessWidget {
  ...
  String get _emergencyName {
    switch (emergency) {
      case EmergencyType.mpox: return "Mpox";
      case EmergencyType.ebola: return "Ebola";
      case EmergencyType.sars: return "SARI";
    }
  }
  ...
  Widget _buildFacilityItem({required BuildContext context, required String title, required String subtitle, required FacilityType type, required bool isImplemented}) {
    return Container(
      ...
      child: ListTile(
        ...
        subtitle: Padding(padding: const EdgeInsets.only(top: 6.0), child: Text(subtitle, style: TextStyle(color: Colors.grey.shade600, fontSize: 13))),
